...
export default function OurStoryPage() {
  const [ourStoryPageData, setOurStoryPageData] = useState(null);
    ...
    async function fetchData() {
      try {
        ...
        const [pageData, seo] = await Promise.all([
          getPageData("about-us/our-story"),
          formatSeoMeta("about-us/our-story"),
        ]);
  ...
  return (
    <>
      {/* SEO META */}
      <Helmet>
        <title>{seoData?.title || "Our Story"}</title>
        {seoData?.description && (
      ...
      <RenderBlocksHelper blocks={ourStoryPageData} />
    </>
  );
}
